...
// Render Region Trend Table
function renderTrendTable() {
    let headerTr = document.getElementById('trend-table-header');
    let bodyTbody = document.getElementById('trend-table-body');
    ...
    rowsToRender.forEach(rowName => {
        let tr = document.createElement('tr');
        if (rowName === 'Toàn quốc') {
            tr.className = 'national-row';
        }

        let tdName = document.createElement('td');
        tdName.innerText = rowName;
        tr.appendChild(tdName);

        dateRange.forEach(date => {
            let histList = appData.history[date] || [];
            let rData = histList.find(h => h.TenVung === rowName);
            let rate = rData ? rData.TyLeTuNop : 0;
            ...
            if (rate < 60) {
                ...
                tdRate.style.color = 'var(--color-green)';
            }
...
function renderCharts(compareRegions = []) {
    ...
    if (compareRegions.length === 0) {
        ...
        if (compareSelect) {
            ...
            compareRegions = Array.from(checkedInputs).map(input => input.value);
        }
    }
    if (compareRegions.length === 0) {
        let user = getActiveUser();
    ...
    let datasets = compareRegions.map((regionName, idx) => {
        ...
        return {
            ...
            data: dataPoints,
            borderColor: color,
            backgroundColor: isNational ? 'rgba(255, 255, 255, 0.03)' : color + '15',
            borderWidth: isNational ? 3 : 2,
            borderDash: isNational ? [5, 5] : [],
            tension: 0.3,
...
const ghnEmailMappings = {
    "dangnh@ghn.vn": { role: "admin", region: "Toàn quốc", name: "Nguyễn Hải Đăng" },
    "haidang@ghn.vn": { role: "admin", region: "Toàn quốc", name: "Nguyễn Hải Đăng" },
